...
const BookmarkSelection: HTMLSelectElement = <HTMLSelectElement> document.getElementById("BookmarkSelection");
const FaviconDisplay: HTMLImageElement = <HTMLImageElement> document.getElementById("WebsiteFavicon");
const UpdateButton: HTMLInputElement = <HTMLInputElement> document.getElementById("UpdateButton");
const MessageToUserLabel: HTMLFadingParagraphElement = <HTMLFadingParagraphElement> document.getElementById("MessageToUserLabel");
const UpdateTitleCheckbox: HTMLInputElement = <HTMLInputElement> document.getElementById("UpdateTitleCheckbox");
const SettingsIcon: HTMLImageElement = <HTMLImageElement> document.getElementById("SettingsIcon");
...
let previousBookmark:chrome.bookmarks.BookmarkTreeNode|null = null; 


chrome.storage.sync.get(["Options"], (items) => {
	// getting the options from storage keeping in mind that it could be undefined
	let options:ExtensionOptions | undefined = items.Options;
	...
	chrome.tabs.query(
		...
		(tabs) => 
		{
			...
			//console.log(tabs);
			FaviconDisplay.src = <string> tabs[0].favIconUrl;
			
            // Populate selection field with the name of the bookmarks the same website as this tab
			
			// Look for that kind of bookmark
			const regex =  /:\/\/\S*?\/|:\/\/\S*/g;
			const tabURL: string = <string> tabs[0].url;
            
			chrome.bookmarks.search((<RegExpExecArray>regex.exec(tabURL))[0], (results)=> {
				...
				let sorter:(a:chrome.bookmarks.BookmarkTreeNode,b:chrome.bookmarks.BookmarkTreeNode)=>number;
				if(options) {
...
function UpdateBookmark():void
{
	...
	(tabs) => 
	{
		...
		let NewTitleAndUrl: chrome.bookmarks.BookmarkChangesArg = {
			url: tabs[0].url
		}
		...
		chrome.bookmarks.update(BookmarkToUpdateID, 
			NewTitleAndUrl,
			(result) => 
			{
 ...
 */
function FadeOut(fadingParagraph:HTMLFadingParagraphElement, fadingDuration:number, frameLength:number = 30):boolean {
    if (fadingParagraph===null) throw "fadingParagraph can't be null";
    if (fadingParagraph.isFading === true) return false;
    fadingParagraph.isFading = true;

    const intervalID:number = setInterval(()=>{
        const opacityValue:number = parseFloat(<string>fadingParagraph.style.opacity)
        if (opacityValue > 0) {
            // math to reduce opacity based on framelength and fading duration
            const newOpacity:number =
                opacityValue - (frameLength / fadingDuration);
 ...
 */
function DisplayMessageToUser(message:string, backgroundColor:string, textColor:string, setOpacityTo:number=1):void {
...
function DetectCTRLZ(ev:KeyboardEvent) {
...
function Undo() {
    // null handling
    if (previousBookmark === null) return;
    ...
    if (previousBookmark.url) changes.url = previousBookmark.url;


    chrome.bookmarks.update(previousBookmark.id, changes, results => {
        // Change the text on the selected option
        SelectedOption.text = (<chrome.bookmarks.BookmarkTreeNode>previousBookmark).title;
